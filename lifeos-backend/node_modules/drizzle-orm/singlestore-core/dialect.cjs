...
var SingleStoreDialect = class {
	...
	async migrate(migrations, session, config) {
		...
		const dbMigrations = await session.all(__sql_sql_ts.sql`select id, hash, created_at from ${__sql_sql_ts.sql.identifier(migrationsTable)} order by created_at desc limit 1`);
		if (typeof config === "object" && config.init) {
			...
			if (!migration) return;
			await session.execute(__sql_sql_ts.sql`insert into ${__sql_sql_ts.sql.identifier(migrationsTable)} (\`hash\`, \`created_at\`) values(${migration.hash}, ${migration.folderMillis})`);
			return;
		}
		...
		await session.transaction(async (tx) => {
			for (const migration of migrations) if (!lastDbMigration || Number(lastDbMigration.created_at) < migration.folderMillis) {
				...
				await tx.execute(__sql_sql_ts.sql`insert into ${__sql_sql_ts.sql.identifier(migrationsTable)} (\`hash\`, \`created_at\`) values(${migration.hash}, ${migration.folderMillis})`);
			}
		});
	}
	escapeName(name) {
	...
	buildSelection(fields, { isSingleTable = false } = {}) {
		...
		const chunks = fields.flatMap(({ field }, i) => {
			...
			if ((0, __entity_ts.is)(field, __sql_sql_ts.SQL.Aliased) && field.isSelectionField) chunk.push(__sql_sql_ts.sql.identifier(field.fieldAlias));
				...
				if (isSingleTable) chunk.push(new __sql_sql_ts.SQL(query.queryChunks.map((c) => {
