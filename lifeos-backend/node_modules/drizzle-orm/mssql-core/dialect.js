...
var MsSqlDialect = class {
	...
	async migrate(migrations, session, config) {
		...
		const { recordset: dbMigrations } = await session.execute(sql`select id, hash, created_at from ${sql.identifier(migrationsSchema)}.${sql.identifier(migrationsTable)} order by created_at desc offset 0 rows fetch next 1 rows only`);
