...
var MsSqlDialect = class {
	...
	async migrate(migrations, session, config) {
		...
		const { recordset: dbMigrations } = await session.execute(__sql_sql_ts.sql`select id, hash, created_at from ${__sql_sql_ts.sql.identifier(migrationsSchema)}.${__sql_sql_ts.sql.identifier(migrationsTable)} order by created_at desc offset 0 rows fetch next 1 rows only`);
